"use strict";
var __rest = (this && this.__rest) || function (s, e) {
    var t = {};
    for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p) && e.indexOf(p) < 0)
        t[p] = s[p];
    if (s != null && typeof Object.getOwnPropertySymbols === "function")
        for (var i = 0, p = Object.getOwnPropertySymbols(s); i < p.length; i++) {
            if (e.indexOf(p[i]) < 0 && Object.prototype.propertyIsEnumerable.call(s, p[i]))
                t[p[i]] = s[p[i]];
        }
    return t;
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.default = RenderHTML;
const react_1 = __importDefault(require("react"));
const RenderHTMLDebug_1 = __importDefault(require("./RenderHTMLDebug"));
const TRenderEngineProvider_1 = __importDefault(require("./TRenderEngineProvider"));
const RenderHTMLConfigProvider_1 = __importDefault(require("./RenderHTMLConfigProvider"));
const RenderHTMLSource_1 = __importDefault(require("./RenderHTMLSource"));
/**
 * Render HTML text in native views!
 *
 * @remarks If your application uses many instances of this component, you
 * should share the render engine across those instances via the
 * {@link TRenderEngineProvider} and {@link RenderHTMLConfigProvider}
 * components, and render the HTML with {@link RenderHTMLSource} instead. That
 * should significantly increase performance.
 *
 * @warning It should never be a child of a React Native `Text` element, as the
 * layout will break.
 *
 * @public
 */
function RenderHTML(props) {
    const { source, onHTMLLoaded, onTTreeChange, onDocumentMetadataLoaded, contentWidth } = props, otherProps = __rest(props, ["source", "onHTMLLoaded", "onTTreeChange", "onDocumentMetadataLoaded", "contentWidth"]);
    return (react_1.default.createElement(RenderHTMLDebug_1.default, Object.assign({}, props),
        react_1.default.createElement(TRenderEngineProvider_1.default, Object.assign({}, otherProps),
            react_1.default.createElement(RenderHTMLConfigProvider_1.default, Object.assign({}, otherProps), react_1.default.createElement(RenderHTMLSource_1.default, {
                source,
                onHTMLLoaded,
                onTTreeChange,
                onDocumentMetadataLoaded,
                contentWidth
            })))));
}
